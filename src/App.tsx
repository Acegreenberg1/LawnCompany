...
import LawnMowingBundle from './LawnMowingBundle';
    ...
    <div className="min-h-screen bg-[#f5f1e8] text-[#4a4a4a]">
      ...
      <section className="min-h-screen flex flex-col justify-center items-start px-8 md:px-16 lg:px-32 py-16 pt-32">
        ...
        <p className="text-xl md:text-2xl text-[#4a4a4a] mb-12 max-w-3xl">
          We provide top quality lawn care in the Kerrville and Ingram area. Call Kerrville Lawn Company at (830) 383-0189 for a quote on your lawn. 112 Holly Hill Dr, Kerrville, TX 78028
        </p>
        <button
...
function LawnMowingBundlePage() {
  const [isContactModalOpen, setIsContactModalOpen] = useState(false);
  return (
    <>
      <Header onNavigate={() => {}} currentPage="services" onOpenContact={() => setIsContactModalOpen(true)} />
      <LawnMowingBundle onOpenContact={() => setIsContactModalOpen(true)} />
      <Footer />
      <ContactModal isOpen={isContactModalOpen} onClose={() => setIsContactModalOpen(false)} />
    </>
  );
